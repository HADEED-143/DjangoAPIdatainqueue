...
2. Add data to a queue.           Done
3. Apply LIFO and FIFO on the data, without using any library or function.     Done
...
class casesList(APIView):
    def get(self, request):
        cases1 = cases.objects.all().order_by('id')
        serializer_class = casesSerializer(cases1, many=True)
        queuedata = [data for data in cases1]   #Add Data in Queue And FIFO implement Ascending order here
        data = {"success": queuedata}
        return Response(serializer_class.data)
    def post(self):
        pass
class labList(APIView):
    def get(self, request):
        labs1 = labs.objects.all().order_by('-id')
        serializer_class = labsSerializer(labs1, many=True)
        queuedata = [data for data in labs1]
        data = {"success": queuedata}
...
class quarantineList(APIView):
    def get(self, request):
        qurantine1 = quarantine.objects.all().order_by('-id')
        serializer_class = quarantineSerializer(qurantine1, many=True)
        filter_backends = [DjangoFilterBackend]
        filterset_fields = ['id']
        queuedata = [data for data in qurantine1]
        data = {"success": queuedata}
        return Response(serializer_class.data)
...
class sqliteseqList(APIView):
    def get(self, request):
        ...
        queuedata = [data for data in sqliteseq1]
        data = {"success": queuedata}
        return Response(serializer_class.data)
...
class summeryList(APIView):
    def get(self, request):
        ...
        queuedata = [data for data in summery1]
        data = {"success": queuedata}
        return Response(serializer_class.data)
...
class vaccineList(APIView):
    def get(self, request):
        ...
        filter_backends = [DjangoFilterBackend]
        filterset_fields = ['id']
        queuedata = [data for data in vaccine1]
        data = {"success": queuedata}
        return Response(serializer_class.data)
...
'''sampledata = [casesList]
        for key in casesList:
            sampledata.pop([key])# Data will Receive in queue and also implement FIFO
                return Response(casesList)'''

"""  serializer_class = labsSerializer(labs1, many=True)
        queuedata = [data for data in labs1]
        sampledata = queuedata[labs1]
        sampledata.pop(queuedata)
        return Response(serializer_class.sampledata)
"""
